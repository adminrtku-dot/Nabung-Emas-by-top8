...
// 2. INITIALIZE SPREADSHEET DAN SHEET JIKA BELUM ADA
...
// 3. AMBIL SEMUA DATA APLIKASI (GET APP DATA)
function getAppData() {
  try {
    ...
    for (let i = 1; i < settingsRows.length; i++) {
      ...
      // Konversi angka jika memungkinkan
      if (!isNaN(val) && val !== "") {
        val = Number(val);
      }
      settings[key] = val;
    ...
    for (let i = 1; i < goalsRows.length; i++) {
      goals.push({
        ...
        targetGram: Number(goalsRows[i][2]),
        warna: String(goalsRows[i][3])
      });
    }
    ...
    for (let i = 1; i < txRows.length; i++) {
      transactions.push({
        ...
        tanggal: Utilities.formatDate(new Date(txRows[i][1]), Session.getScriptTimeZone(), "yyyy-MM-dd"),
        tipe: String(txRows[i][2]),
        gram: Number(txRows[i][3]),
        hargaSatuan: Number(txRows[i][4]),
        totalRp: Number(txRows[i][5]),
...
// 12. RESET SELURUH DATA APLIKASI
function resetAllData() {
  try {
    ...
    // Jalankan ulang inisialisasi awal
    getSpreadsheet();
